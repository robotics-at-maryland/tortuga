 ...
 * File:   tools/vision_viewer/src/GLMovie.cpp
 */
...
void GLMovie::render()
{
    ...
    // Samples seem to prefer Update to Refresh.  Update also seems to correctly
    // resize the OpenGL drawing area.  When using Refresh, the glcanvas is
    // always one size behind.
    Update();
    //Seems to occur in all samples.
    SetCurrent();
    //Seems to occur in all samples.
...
void GLMovie::copyFrom(const Image* src)
{
    ...
    if (width != this->movieWidth && height != this->movieHeight)
    {
        ...
        delete[] this->imageData;
        this->imageData =
            new unsigned char[this->imageWidth * this->imageHeight * 3];
    }
