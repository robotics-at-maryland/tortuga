                                 ...
                                 math::Vector3& rotationalTorqueOut)
{
    ...
    m_desiredState->setDesiredDepth(dd);
        ...
        if((vConz == false && m_desiredState->vz == true))
        {
            ...
            intTermz = m_depthController->getISum()/kivz; //steal the positional controllers z integral term
            //currently this is hacked to account for depth controller's k_i constant, fix me later!(k_i for depth control is 1)
        }
