function [s, v, a, j] = SVAJ_generator(deltat, act_depth,des_depth,vel_max)
...
step = 0;
range = 0 : deltat : total_time;
s = zeros(length(range),1);
v = zeros(length(range),1);
a = zeros(length(range),1);
j = zeros(length(range),1);

for current_time = range
step=step+1;
...
a(step) = (change_in_depth)*  (Cp*12*(1/total_time)^2*(current_time/total_time)^2 + Cq*20*(1/total_time)^2*(current_time/total_time)^3 + Cr*30*(1/total_time)^2*(current_time/total_time)^4 + Cs*42*(1/total_time)^2*(current_time/total_time)^5);
j(step) = (change_in_depth)*  (Cp*24*(1/total_time)^3*(current_time/total_time) + Cq*60*(1/total_time)^3*(current_time/total_time)^2 + Cr*150*(1/total_time)^3*(current_time/total_time)^3 + Cs*210*(1/total_time)^3*(current_time/total_time)^4);
end
