...
if strcmp(method,'canny')
  ...
  sz = size(size(a));
  if sz(2) == 3
      ch1x = imfilter(aSmooth(:,:,1), dgau2D, 'conv','replicate');
      ch1y = imfilter(aSmooth(:,:,1), dgau2D', 'conv','replicate');
      
      ch2x = imfilter(aSmooth(:,:,2), dgau2D, 'conv','replicate');
      ch2y = imfilter(aSmooth(:,:,2), dgau2D', 'conv','replicate');
      
      ch3x = imfilter(aSmooth(:,:,3), dgau2D, 'conv','replicate');
      ch3y = imfilter(aSmooth(:,:,3), dgau2D', 'conv','replicate');
...
% If it's a three channel image, check it differently
sz = size(size(I));
if sz(2) == 3
    % Three channel image
    iptcheckinput(I(:,:,1),{'numeric','logical'},{'nonsparse','2d'}, ...
                  mfilename,'I',1)
    iptcheckinput(I(:,:,2),{'numeric','logical'},{'nonsparse','2d'}, ...
                  mfilename,'I',1)
    iptcheckinput(I(:,:,3),{'numeric','logical'},{'nonsparse','2d'}, ...
                  mfilename,'I',1)
else
    iptcheckinput(I,{'numeric','logical'},{'nonsparse','2d'}, ...
                  mfilename,'I',1);
end
...
% Only perform multi-dimensional filtering on canny
if (sz(2) == 3) && (~strcmp(Method, 'canny'))
    error(['Cannot do multi-dimensional filtering for anything other ' ...
           'than canny']);
end
