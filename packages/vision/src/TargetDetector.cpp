...
logger.infoStream() <<"Erode "<< m_erodeIterations <<" Dilate "<< m_dilateIterations;
	...
	Mat temphsv;
	cvtColor(img,temphsv,CV_BGR2HSV);
	...
	cvtColor(temphsv,img,CV_HSV2BGR);
...
TargetDetector::targetPanel TargetDetector::getSquareBlob(Mat erosion_dst, Mat img_whitebalance, Color::ColorType color )
{
	...
	else	
	{
		...
		double minY= 90000;
		double maxY=0;	
		double Mainwidth=0,Mainheight=0;
		Mainwidth=maxtemp.size.width;
		Mainheight=maxtemp.size.height;
		for (int i = 0; i < 4; i++)
		{
...
//minEllipse.size.width<Mainwidth && minEllipse.size.height <Mainheight
				{
