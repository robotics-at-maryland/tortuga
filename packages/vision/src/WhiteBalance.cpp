...
//	cout <<"ColSUm " << ColSum;
	...
	float tempb_b = tempb.val[output_plane_blue];
	float tempb_g = tempb.val[output_plane_green];
	float tempb_r = tempb.val[output_plane_red];
...
//switch from output_planes from 0,1,2, to 2,1,0 since I think it maybe in BGR instead of RGB
	output_planes[output_plane_blue] = output_planes[output_plane_blue]*scale_b;
	output_planes[output_plane_green] = output_planes[output_plane_green]*scale_g;
	output_planes[output_plane_red] = output_planes[output_plane_red]*scale_r;
