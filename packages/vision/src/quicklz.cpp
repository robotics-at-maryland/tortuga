...
#if QLZ_VERSION_MAJOR != 1 || QLZ_VERSION_MINOR != 4 || QLZ_VERSION_REVISION != (-6)
...
static __inline void update_hash(qlz_hash_entry h[QLZ_HASH_VALUES], unsigned char counter[QLZ_HASH_VALUES], const unsigned char *s)
{
...
static void update_hash_upto(qlz_hash_entry h[QLZ_HASH_VALUES], unsigned char counter[QLZ_HASH_VALUES], unsigned char **lh, const unsigned char *max)
{
	while(*lh < max)
	{
		(*lh)++;
...
static size_t qlz_compress_core(const unsigned char *source, unsigned char *destination, size_t size, qlz_hash_entry hashtable[QLZ_HASH_VALUES], unsigned char hash_counter[QLZ_HASH_VALUES])
{
	...
	while(src <= last_matchstart)
	{
...
#if QLZ_COMPRESSION_LEVEL == 1
		{
...
#endif
			{
				...
				else
				{
					...
					size_t remaining, matchlen;

					remaining = (last_byte - UNCOMPRESSED_END - src + 1) > 255 ? 255 : (last_byte - UNCOMPRESSED_END - src + 1);
			...
			{
				hash_counter[hash] = 1; // todo, test if decreasing comp ratio on small input
				*dst = *src;
				src++;
				dst++;
				...
				fetch = fast_read(src, 3);
#else
				fetch = fetch >> 8 & 0xffff | *(src + 2) << 16; // fast_read(src, 3);
#endif
			}
		}
#elif QLZ_COMPRESSION_LEVEL >= 2
		{
			...
			if(offset2 < src - MINOFFSET && c > 0 && ((fast_read(offset2, 3) ^ fetch) & 0xffffff) == 0) // erstattet 2 med MINOFFSET
			{	
				matchlen = 3;
			...
			if(matchlen > 2)
			{
				...
				if (matchlen < 10)
				{			
					ui32 f = best_k | ((matchlen - 2) << 1) | (hash << 4);
					fast_write(f, dst, 2);
					dst += 2;
				}
				else
				{
					ui32 f = best_k | (matchlen << 16) | (hash << 4);
					fast_write(f, dst, 3);
					dst += 3;
				}
			}
...
#if QLZ_COMPRESSION_LEVEL < 3
		if (src <= last_byte - 3)
			update_hash(hashtable, hash_counter, src);
#endif
		*dst = *src;
		src++;
		dst++;
...
static size_t qlz_decompress_core(const unsigned char *source, unsigned char *destination, size_t size, qlz_hash_entry hashtable[QLZ_HASH_VALUES], unsigned char hash_counter[QLZ_HASH_VALUES], unsigned char *history, const char *source_2)
{
	const unsigned char *src = source;
	...
	const unsigned char *last_matchstart = last_destination_byte - UNCOMPRESSED_END + 1;
	unsigned char *last_hashed = destination - 1;
	const unsigned char *last_source_byte = source + qlz_size_compressed(source_2) - 1;
...
size_t qlz_compress(const void *source, char *destination, size_t size, char *scratch_compress)
{
	...
	qlz_hash_entry *hashtable = (qlz_hash_entry *)(scratch_aligned + QLZ_BUFFER_COUNTER);
	unsigned char *hash_counter = (unsigned char*)hashtable + sizeof(qlz_hash_entry[QLZ_HASH_VALUES]);
...
// if QLZ_STREAMING_BUFFER == 0 and first case (streaming buffer full) is executed unconditionally, functioning as block comp.
...
size_t qlz_decompress(const char *source, void *destination, char *scratch_compress)
{
	...
	qlz_hash_entry *hashtable = 0;
#elif QLZ_COMPRESSION_LEVEL <= 2
	qlz_hash_entry *hashtable = (qlz_hash_entry *)(scratch_aligned + QLZ_BUFFER_COUNTER); 
	unsigned char *hash_counter = (unsigned char*)hashtable + sizeof(qlz_hash_entry[QLZ_HASH_VALUES]);
	unsigned char *streambuffer = hash_counter + QLZ_HASH_VALUES; 
#endif
	ui32 headerlen = 2*((((*source) & 2) == 2) ? 4 : 1) + 1; // get header len
	size_t dsiz = qlz_size_decompressed((char *)source);

	if (*buffersize + qlz_size_decompressed((char *)source) - 1 >= QLZ_STREAMING_BUFFER) 
	{	
		if((*source & 1) == 1)
		{
#if QLZ_COMPRESSION_LEVEL != 3
