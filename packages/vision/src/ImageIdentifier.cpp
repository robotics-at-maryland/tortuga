...
ram::vision::ImageIdentifier::ImageIdentifier (const unsigned int images, const unsigned int imageHeight, const unsigned int imageWidth):
m_canTrain (true), m_maxIndex (0), m_trainIndex (0) {
...
ram::vision::ImageIdentifier::ImageIdentifier (const boost::filesystem::path &file, bool loadTrainingData): m_canTrain (false) {
	boost::filesystem::path net = file;
	boost::filesystem::path training = file;
	boost::filesystem::change_extension (training, "trn");
	...
	if (loadTrainingData) {
		if (boost::filesystem::exists (training)) {
...
int ram::vision::ImageIdentifier::addTrainData (const std::vector<ram::vision::Image*> &images) {
	if (!m_canTrain || images.size() == 0) {
		return -1;
	}
	IplImage* gray;
	...
	if (m_data.length_train_data() == 0) {
		m_data.set_train_data (images.size(), m_net.get_num_input(), input, m_net.get_num_output(), output);
	} else {
...
void ram::vision::ImageIdentifier::runTraining () {
...
const void ram::vision::ImageIdentifier::runTest () {
	float MSE = 0.0;
	m_net.reset_MSE();
...
int ram::vision::ImageIdentifier::run (ram::vision::Image* input) {
...
const bool ram::vision::ImageIdentifier::save (const boost::filesystem::path &file, bool saveTrainingData) {
	bool value;
	boost::filesystem::path net = file;
	boost::filesystem::path training = file;
	if (boost::filesystem::extension (net) != "irn") {
		...
		if (saveTrainingData) {
			training = boost::filesystem::path (training.file_string() + ".trn");
...
void ram::vision::ImageIdentifier::loadImage (IplImage* src, fann_type* target) {
	unsigned int u = 0;
...
IplImage* ram::vision::ImageIdentifier::grayscale (IplImage* src) {
