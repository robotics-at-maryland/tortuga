...
class RAM_EXPORT ImageIdentifier {
  public:
    ...
    ImageIdentifier (const unsigned int images, const unsigned int imageHeight,
    ...
    inline const fann_type resultValue (unsigned int index) {
        return m_outValue[index]; }
  ...
  private:
    ...
    inline static const fann_type getPixel (IplImage* src, unsigned int w,
                                            unsigned int h) {
        return (src->imageData + (w * src->widthStep))[h]; }
