...
# File:  packages/vision/python/hedge.py
#
# Requirements:
...
#         - python-opencv
#         - python-yaml
#   - Must have a ~/stock_images/hedge folder
...
def analyze(filename, cfg, output_cfg):
    ...
    if len(blobList) > 0:
        # Choose the largest square as the found hedge
        blob = blobList[0]
...
def main():
    ...
    source = os.path.expanduser(os.path.join('~', 'stock_images', 'hedge', 'ppm'))
