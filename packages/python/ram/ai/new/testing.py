...
class TestMachine(StateMachine):
    def __init__(self):
        ...
        #center = self.addState('center', motionStates.Turn(135))
        #align = self.addState('align', centering.DownOrient(pipe, 'end', 'end'))
        acoustic = self.addState('acoustic', acousticServoing.AcousticServoing(pinger, math.Vector3(0.0,0.0,3.0), .5, .5))
        start.setTransition('next', 'acoustic')
        acoustic.setTransition('next','end')
