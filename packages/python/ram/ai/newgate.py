...
class FindGate(state.State):
    ...
    @staticmethod
    def transitions():
        return { vision.EventType.GATE_FOUND : AcquireBoth,
                 motion.basic.MotionManager.FINISHED : Forward }

    @staticmethod
    def getattr():
        global searchDistance
        return { 'avgRate' : 0.15, 'distance' : searchDistance }
...
BOTH_FOUND = core.declareEventType('BOTH_FOUND')
STILL_NOT_FOUND = core.declareEventType('STILL_NOT_FOUND')
GO_LEFT = core.declareEventType('GO_LEFT')
GO_RIGHT = core.declareEventType('GO_RIGHT')
global directionCount
directionCount = 1
# start of AcquireBoth
class AcquireGate(utility.MotionState):

    @staticmethod
    def transitions():
        return { BOTH_FOUND : GateApproach,
                 STILL_NOT_FOUND : AcquireGate,
                 vision.EventType.GATE_FOUND : AcquireGate,
                 vision.EventType.GATE_LOST : AcquireGate,
                 GO_LEFT : AcquireGate}

    @staticmethod
    def getattr():
        return { 'strafeDist' : 0.1 }
    ...
    def GATE_FOUND(self, event):
        if event.azimuth == 1:
            self.publish(STILL_NOT_FOUND, core.Event())
        else:
            self.publish(BOTH_FOUND, core.Event())
