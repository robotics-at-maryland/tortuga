...
HOLD = core.declareEventType('HOLD')
IMU_KILL = core.declareEventType('IMU_KILL')

IMU_UNKILL = core.declareEventType('IMU_UNKILL')
...
class FindGate(state.State):
    ...
    def getattr():
        ...
        return { 'avgRate' : 0.1, 'distance' : searchDistance }
    ...
    def enter(self):
        ...
        self.timer = self.timerManager.newTimer(IMU_KILL, 
                                                75)
        self.timer.start()

        self.timerb = self.timerManager.newTimer(IMU_UNKILL, 
                                                 120)
        self.timerb.start()
        
        self.publish(HOLD, core.Event())

    def exit(self):
        utility.freeze(self)
        self.motionManager.stopCurrentMotion()

        print 'Uncorrupting IMU'
        self.vehicle.corruptIMU(False)

    def IMU_KILL(self, event):
        print 'Corrupting IMU'
        self.vehicle.corruptIMU(True)

    def IMU_UNKILL(self, event):
        print 'Uncorrupting IMU'
        self.vehicle.corruptIMU(False)

    def HOLD(self, event):
        self.controller.holdOrientation()
        self.holder = self.timerManager.newTimer(HOLD, 0.1)
        self.holder.start
