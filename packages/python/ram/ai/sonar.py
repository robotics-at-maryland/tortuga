...
class Searching(state.State):
    ...
    def UPDATE(self, event):
        if self._first:
            pingerOrientation = ext.math.Vector3.UNIT_X.getRotationTo(
                event.direction)
            #self.controller.yawVehicle(pingerOrientation.getYaw(True).valueDegrees(), 0.1)
            
            pingerYaw = pingerOrientation.getYaw(True).valueDegrees()

            currentOrientation = self.stateEstimator.getEstimatedOrientation()

            orientation = yawVehicleHelper(currentOrientation, pingerYaw)
