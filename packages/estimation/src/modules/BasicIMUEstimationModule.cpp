...
BasicIMUEstimationModule::BasicIMUEstimationModule(core::ConfigNode config,
    ...
    m_gyroXThresh(config["xThresh"].asDouble(400)),//17
    m_gyroYThresh(config["yThresh"].asDouble(100)),
    m_gyroZThresh(config["zThresh"].asDouble(10))
{
...
void BasicIMUEstimationModule::update(core::EventPtr event)
{
    ...
    else
    {
        ...
        math::Vector3 omega(0,0,0);
        {
            ...
            std::cout<<"dgyro/dt: "<<omega<<std::endl<<m_filteredGyroXV.getValue()<<" "<<m_filteredGyroYV.getValue()<<" "<<m_filteredGyroZV.getValue() <<"FF"<<std::endl;
        }
