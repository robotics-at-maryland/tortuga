...
static log4cpp::Category& LOGGER(log4cpp::Category::getInstance("StEstVision"));
...
{
    vision::VisionEventPtr visionEvent = 
    ...
    // if the obstacle is touching the edge the distance estimate will be dangerous
    // we want to avoid this situation even if it means ignoring a found event
    // if(visionEvent->touchingEdge)
    //     return;

    // form the measurement in the image coordinate frame
    math::Vector3 measurement_i(visionEvent->x * m_camWidth,
                                visionEvent->y * m_camHeight,
                                visionEvent->range);
...
double ParticleVisionEstimationModule::likelihood2D(
...
double ParticleVisionEstimationModule::likelihood3D(
...
double ParticleVisionEstimationModule::gaussian1DLikelihood(
