 ...
 * File:  packages/sonar/src/PingerTracker.cpp
 */

/* 
 * This is the main program to run the sonar.
 * Well, actually, it once was.  Now it is the main program's zombie
 * stepchild with multiple personality disorder.
 *
 * PingTracker_run will be invoked twice, on separate threads, with access to the
 * ADCs and the CPU reporting arbitrated by mutexes.
 *
 * Here is how the original program worked.
...
void* PingerTracker_run(void* inArgsBuf)
{
    ...
    int myKBands[nKBands] = {inArgs[0], inArgs[1]};
