 ...
 * File:  packages/packages/vehicle/test/src/TestLoopStateEstimator.cxx
 */
                       ...
                       Quaternion magAccelRotation))
{
    ...
    ram::vehicle::device::LoopStateEstimator::quaternionFromIMU(
        vehicleMag.ptr(), vehicleAccel.ptr(), result);
...
TEST(quaternionFromRate) {
  ...
  //run the function
  ram::vehicle::device::LoopStateEstimator::quaternionFromRate(
      qOld,w,deltaT,qOut);
  ...
  ram::vehicle::device::LoopStateEstimator::quaternionFromRate(
      qOld2,w2,deltaT2,qOut2);
  CHECK_ARRAY_CLOSE(qExp2,qOut2,4,0.001);

}

// TODO: Figure out what this was supposed to test and how to update it
