...
MacKeyboard::~MacKeyboard()
{
    ...
    // after it is deleted
    if ( keyDownEventRef != NULL ) RemoveEventHandler( keyDownEventRef );
    if ( keyUpEventRef != NULL ) RemoveEventHandler( keyUpEventRef );
    if ( keyModEventRef != NULL ) RemoveEventHandler( keyUpEventRef );
...
void MacKeyboard::_initialize()
{
    ...
    EventTypeSpec DownSpec[2];
    EventTypeSpec UpSpec, ModSpec, RepeatSpec;
    
    // Set the type of events we wish to handle
    DownSpec[0].eventClass = kEventClassKeyboard;
    DownSpec[0].eventKind = kEventRawKeyDown;
    DownSpec[1].eventClass = kEventClassKeyboard;
    DownSpec[1].eventKind = kEventRawKeyRepeat;
    
    UpSpec.eventClass = kEventClassKeyboard;
    UpSpec.eventKind = kEventRawKeyUp;
    
    // Separate events for modifiers change, because they aren't reported normally
    ...
    if ( useRepeat ) {
        ...
        status = InstallWindowEventHandler( window, keyDownUPP, 2, DownSpec, this, &keyDownEventRef );
    }
    else {
        status = InstallWindowEventHandler( window, keyDownUPP, 1, DownSpec, this, &keyDownEventRef );
    }
    ...
    status = InstallWindowEventHandler( window, keyUpUPP, 1, &UpSpec, this, &keyUpEventRef );
    if ( status != noErr ) {
        OIS_EXCEPT( E_General, "MacKeyboard::_initialize >> Error loading KeyUp event handler" );
    }
    
    status = InstallWindowEventHandler( window, keyModUPP, 1, &ModSpec, this, &keyModEventRef );
    if ( status != noErr ) {
...
void MacKeyboard::capture()
{
    ...
    for (cur_it = pendingEvents.begin(); cur_it != pendingEvents.end(); cur_it++) {
        ...
        if ( (*cur_it).Type == MAC_KEYDOWN || (*cur_it).Type == MAC_KEYREPEAT) {
            listener->keyPressed( (*cur_it).Event );
        } 
        else if ( (*cur_it).Type == MAC_KEYUP ) {
            listener->keyReleased( (*cur_it).Event );
        }
    }
...
void MacKeyboard::_keyDownCallback( EventRef theEvent )
{
    ...
    if (mTextMode == Unicode) {
        ...
        //get string size
        UInt32 stringsize;
        status = GetEventParameter( theEvent, 'kuni', typeUnicodeText, NULL,
                                    0, &stringsize, NULL);
        
        //cout << "String length: " << stringsize << endl;
        
        status = GetEventParameter( theEvent, 'kuni', typeUnicodeText, NULL,
                                    sizeof(UniChar)*10, NULL, &text );
        //wstring unitext;
        ...
        status = GetEventParameter( theEvent, 'kchr', typeChar, NULL,
                                    sizeof(char), NULL, &macChar );
