...
bool Win32Mouse::_doMouseClick( int mouseButton, DIDEVICEOBJECTDATA& di )
{
	if( di.dwData & 0x80 )
	{
		...
		if( listener && mBuffered )
			return listener->mousePressed( MouseEvent( this, mState ), (MouseButtonID)mouseButton );
	}
	else
	{
		mState.buttons &= ~(1 << mouseButton); //turn the bit flag off
		if( listener && mBuffered )
			return listener->mouseReleased( MouseEvent( this, mState ), (MouseButtonID)mouseButton );
	}
