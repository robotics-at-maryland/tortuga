...
for rotAngle = 0:2:90
	...
	for i = 1:4
		...
		subplot(2,4,i);plot(abs(fft(sum(suitRot)))(1:16));
		axis off;title(horzcat(suitNames{i}, ', x-projection'));
		subplot(2,4,4+i);plot(abs(fft(sum(suitRot')))(1:16));
		axis off;title(horzcat(suitNames{i}, ', y-projection'));
		print(sprintf('rotFFT%d.pdf',rotAngle),'-dpdf');
	end
end
