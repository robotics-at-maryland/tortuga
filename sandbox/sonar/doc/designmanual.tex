...
Since (\ref{eq:planars-multi}) is a set of linear equations in three variables, the matrix on the left hand side must have rank 3 in order to solve for \(\hat \mathbf k\).  Therefore, we need at least 4 (noncollinear, noncoplanar) hydrophones to determine \(\hat \mathbf k\) using far-field multilateration.
...
If the robot is equipped with two near field type arrays, separated by a fixed distance, then it can apply elementary trigonometry to determine its range using triangulation.
