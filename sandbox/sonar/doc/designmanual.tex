...
\title{The Sonar Cookbook \\ \small{\textsc{An Underwater Acoustics Primer for \TortugaII{}}}}
\author{Leo Singer \\ \small{\textsc{University of Maryland}}}
...
\begin{titlepage}
	\maketitle
	\begin{center}
		\includegraphics[scale=0.5]{robocrab.pdf}
	\end{center}
	\begin{abstract}
...
We will use a technique called \emph{multilateration} to determine our \emph{relative} bearing and/or range from the pinger.  Multilateration estimates the relative position of an acoustic source by comparing time difference on arrival (TDOA) of a pulse sent to a collection of hydrophones at known postitions.
...
In two dimensions, bearing defined as the compass direction of a distant target.  We will demonstrate how to compute \emph{relative} bearing, which is the angle between a fixed axis on the vehicle and a distant target.  In three dimensions, when we say ``relative bearing'' we mean a three-vector that gives the target's direction in vehicle body frame coordinates.

There are two main categories of geometric approximations that give useful results for passive bearing: near field and far field.  In the near field of the pinger, the wavefronts are spherically symmetric, and the curvature can be used to deduce both the bearing and range of the source.

In the far field, the wavefronts are roughly planar, so we can measure the direction of the source but not the distance.

(This is not quite the whole story, because some sound sources might produce waves that are closer to cylindrical than spherical.  In fact, even a spherically symmetric source will produce roughly cylindrical waves in shallow water \cite{urick1996}.)

Although the near field approach is very powerful in that it can be used to immediately pinpoint the location of the source in three dimensions, it is difficult to implement because the range accuracy drops off very rapidly with increasing distance.  This problem can be overcome by making the baseline -- the separation between the hydrophones -- very large, possibly so large that the sensors surround the source of interest.  It would be an intriguing applicaiton for a fleet or ``swarm'' of AUVs.

The far field approximation is much simpler to implement, and is well suited for a small mobile platform such as \TortugaII{}.  What is more, the bearing accuracy will \emph{increase} dramatically as the source becomes more distant!

\subsection{Spherical waves, near field}
\label{sec:near-field}

We will study the near field case first because it will introduce all of the necessary notation.

In the near field of the pinger, where the sound radiates in a radially symmetric pattern, the multilateration problem reduces to finding the intersection point of a collection of hyperboloids, as is shown in figure~(\ref{fig:hyperbola-good}).
...
\subsubsection{Theoretical performance limits}
...
We can estimate the spatial variation in position accuracy for any given sonar array geometry.  Figures (\ref{fig:squarePyramid}) and (\ref{fig:doubleTetrahedron}) depict the positional accuracy of an inverted square pyramid array and a double tetrahedron array, respectively.  We have used MATLAB to compute the Cram\'{e}r-Rao lower bound on the position of the pinger in a number of different locations, then plotted the maximum eigenvalue of the covariance matrix.

\begin{figure}[htbp]
	\begin{center}
		\includegraphics[scale=0.4]{squarePyramid.pdf}
...
\begin{figure}[htbp]
	\begin{center}
		\includegraphics[scale=0.4]{doubleTetrahedron.pdf}
...
\begin{figure}[htbp]
	\begin{center}
		\includegraphics[scale=0.2]{array-cad.jpg}
		\caption{A far-field type 3D bearing array, featuring four hydrophones in a volume-spanning configuration.}
		\label{fig:array-cad}
	\end{center}
\end{figure}
...
\mathbf d_1 & \mathbf d_2 & \dots & d_m\\
1 & 1 & \dots & 1
\end{array}\right) && \\
= \left(\begin{array}{cccccc}
\mathbf k_1 & \mathbf k_2 & \dots & \mathbf k_m\\
1 & 1 & \dots & 1
\end{array}\right) && \\
\left(\begin{array}{cc}
A & -A \mathbf b \\
...
\section{Passive range and navigation}
...
If the robot is equipped with a near field type array (section \ref{sec:far-field}) then the range \(R_s\) is a direct result of the multilateration algorithm, provided the baseline is sufficiently large.  However, range accuracy for the near field approximation drops off dramatically with distance.
...
To estimate TDOAs with phase comparison, each hydrophone must have at least one neighbor less than half a wavelength \(\lambda/2\equiv\frac{\pi}{k}\) distant from it.  In water, a 30 kHz signal has a wavelength of approximately 5 cm.  Obviously, phase comparison severely limits the baseline.

The simplest way to get around this is to use edge detection to get a rough TDOA and then to use phase comparison from the frequency domain to refine it.
...
However, the acoustic environment of TRANSDEC will still suffer from interference due to reflections off the water's surface, producing what is known as a Lloyd's mirror interference pattern \cite{Etter} (figure~\ref{fig:lloyd}).  It will affect perceived amplitude and phase.
...
\begin{figure}[hbt]
	\begin{center}
		\includegraphics[scale=0.8]{lloyd.pdf}
		\caption{The Lloyd mirror effect, after \cite{urick1996} and \cite{Etter}. Log-log plot of sound intensity as a function of distance.  At the sharp dips where \(\cos \pi \mathit{l} / \mathit{l}_0\), the sound intensity goes to zero.}
	\label{fig:lloyd}
	\end{center}
\end{figure}
...
Now, the summations in (\ref{eq:sdft-factored}) and (\ref{eq:sdft-m-minusone}) are identical except for the starting and ending indices.

The sum in (\ref{eq:sdft-factored}) is missing the zeroth term of the sum in (\ref{eq:sdft-m-minusone}), which is \(x(m-N)\).

(\ref{eq:sdft-factored}) also has an \(N\)th term that (\ref{eq:sdft-m-minusone}) is missing, equal to \(x(m) e^{- \frac{2 \pi i}{N} k}\).  Since the argument of the complex exponential in this term is a multiple of \(2 \pi\), this term reduces to \(x(m)\).
...
First, we would like to thank two recently graduated electrical engineering students, Nicholas Limparis and Steve Moskovchenko, for the countless hours they spent designing the DSP hardware for \TortugaII{}'s passive sonar system.

Our thanks go also to Naval Surface Warfare Center, Carderock Division, for lending us hydrophones, a pinger, and test equipment, as well as for providing us with valuable advice in acoustics and analog electronics.

We are also greatful to Dr. Dave Aiken, director of the Space Systems Laboratory (SSL) at the University of Maryland, for letting us do acoustics experiments at the Neutral Buoyancy Research Facility (NBRF), in addition to the design, construction, and testing of \TortugaII{} itself.  The faculty, students, and staff of SSL are a goldmine of expert knowledge on building underwater robots.
