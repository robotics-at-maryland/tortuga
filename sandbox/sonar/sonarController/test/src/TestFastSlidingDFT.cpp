...
TEST_UTILITY(FastSlidingDFTCheckAgainstFFTW, (adcdata_t *adcdataSamples, int nchannels, int countFrames, int k, int N))
{
	...
	FastSlidingDFT myDFT(nchannels, k, N);
	
	double *doubleSamples = new double[countInputData];
	FastSlidingDFTTestFixture::
    ...
    for (int i = N ; i < countFrames ; i ++)
    {
    	...
    	//  reference DFT.  FastSlidingDFT should give equivalent results.
		...
		for (int channel = 0 ; channel < nchannels ; channel ++)
		{
			...
			//  DFT.
			adcmath_t re_theirs = 
				FastSlidingDFTTestFixture::normalize_double(re_propagated);
			adcmath_t im_theirs = 
				FastSlidingDFTTestFixture::normalize_double(im_propagated);
...
TEST_FIXTURE(FastSlidingDFTTestFixture, CompareDFTRandomInput)
{
	...
	TEST_UTILITY_FUNC(FastSlidingDFTCheckAgainstFFTW)(in, nchannels, countFrames, k, N);
