...
def generate_code(module_name, files, output_dir, include_files):
    module_map = {'vehicle' : generate_vehicle,
                  'vehicle::devices' : generate_vehicle_devices}
    ...
    # Now Call into the desired genertor function
    mb.global_ns.namespace('vehicle').include()
...
#    print 'Mod infos:',mod_infos
    ...
    for short_name, full_dot_name, bfiles in mod_infos:
        ...
        lib = envw.SharedLibrary(target = os.path.join(output_dir, short_name),
                                 source = sources)

        # Create init files for importing
        install_path = os.path.join(install_dir,
                                    str(os.sep).join(full_dot_name.split('::')))
        envw.Command(install_dir, '', SCons.Defaults.Touch(os.path.join(install_path, '__init__.py')))
        # Create the extra depends on the output files and generate code files
        # Setup the install command
        envw.InstallAs(target = os.path.join(install_dir, output_path, '_' + short_name + '.so'),
                       source = lib)
