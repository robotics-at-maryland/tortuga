...
.bss kMags,8*64		; must be contiguous after kValue
...
_asmSlidingDFT3:
	...
	; W1 is trigger value *
	; W2 is scratch register ^
	; W3 is kMag pointer max *
...
;*****
	...
	mov		PORTD,W0		; get data
	bset	PORTA,#4		; clock inactive
	asr		W0,#4,W0		; shift right and sign-extend
...
;	mov		[W2++],W0		; get data from ADC
	...
	; load kMag array address
	mov		kMagPtr,W7
	
	; channel 0 norm
	mpy		W6*W6,A,[W8]+=2,W6				; square real part, prefetch kValue(0i) to W7
	mac		W6*W6,A,[W8]+=2,W6				; square and sum imag part, prefetch kValue(1r) to W6
	movsac	B,W13							; write back to kMags
	
	
	; channel 1 norm
	mpy		W6*W6,A,[W8]+=2,W6				; square real part, prefetch kValue(1i) to W6
	mac		W6*W6,A,[W8]+=2,W6				; square and sum imag part, prefetch next real part
	...
	; channel 3 norm
	mpy		W6*W6,A,[W8],W6					; square real part, prefetch kValue(3i)
	mac		W6*W6,A							; square and sum imag part
	movsac	B,[W13]+=2						; write back to kMags
	...
	; >>> Generate trigger slope based on past value
